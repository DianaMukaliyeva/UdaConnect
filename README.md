...
4. `kubectl apply -f deployment/persons-api.yaml` - Set up the service and deployment for the persons API
5. `kubectl apply -f deployment/frontend-udaconnect.yaml` - Set up the service and deployment for the web app
6. `kubectl apply -f deployment/kafka.yaml` - Set up the service and deployment for the Kafka
7. `kubectl apply -f deployment/locations-api.yaml` - Set up the service and deployment for the locations API
8. `kubectl apply -f deployment/locations-consumer.yaml` - Set up the deployment for the locations consumer
...
`kubectl get pods` and `kubectl get services` - should both return `frontend-udaconnect`, `zoo`, `persons-api`, `kafka-service`, `locations-api` and `postgres`
